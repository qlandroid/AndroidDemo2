        ...
        if (mDropListener != null) {
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    int x = (int) ev.getX();
                    int y = (int) ev.getY();
                    int itemnum = pointToPosition(x, y);//获得当前手指所在的listView中的角标position
            ...
            mCoordOffset = ((int) ev.getRawY()) - y;// 获得listView据顶边缘的高度，
            View dragger = item.findViewById(R.id.img);//获得左边图片
            Rect r = mTempRect;//获得矩形框
            ...
            if ((r.left < x) && (x < r.right)) {
                item.setDrawingCacheEnabled(true);//开启缓存
                // Create a copy of the drawing cache so that it does not
                // get recycled
                // by the framework when the list tries to clean up memory
                Bitmap bitmap = Bitmap.createBitmap(item.getDrawingCache());//通过item的缓存获得当前item生成的图片
